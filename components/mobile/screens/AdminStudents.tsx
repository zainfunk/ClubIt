    ...
    <div style={css('position:absolute;inset:0;display:flex;flex-direction:column;animation:scIn .24s ease;')}>
      ...
      {users === null ? <Loader /> : (
        <div className="m-noscroll" style={{ ...css('flex:1;overflow-y:auto;padding:8px 20px 0;'), paddingBottom: `calc(${BOTTOM(0)} + 96px)` }}>
          {students.length === 0 ? <EmptyState title="No students yet" sub="Students who join your school appear here." /> : (
            <div style={css('background:#fff;border:1px solid #eef0f3;border-radius:18px;padding:4px 16px;box-shadow:0 1px 2px rgba(16,24,40,.04);')}>
              {students.map((s, i) => {
                ...
                return (
                  <button key={s.id} onClick={() => router.push(`/students/${s.id}`)} style={css(`width:100%;display:flex;align-items:center;gap:12px;padding:11px 0;border:none;background:none;cursor:pointer;text-align:left;${i < students.length - 1 ? 'border-bottom:1px solid #f4f5f7;' : ''}`)}>
